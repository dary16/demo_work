<template>
  <div class="xl-list clearfix">
    ...
    <div class="xl-header">
      <div class="xl-right" v-show="notNum !== 0">
        <div>
          <el-badge :value="notNum" :max="99" class="badgeNum">
            <button class="normal-btn-border-lg fr" @click="confirmUpload">
              数据上传
            </button>
          </el-badge>
        </div>
...
<script>
  ...
  export default {
    ...
    //监听属性 类似于data概念
    computed: {
      ...mapState(['userInfo', 'userName', 'nowIndex', 'unfinishedTime' ,'unfinishedTimeIndex']),
      //未上传数量
      notNum() {
    ...
    //方法集合
    methods: {
      ...mapMutations(['_nowIndex', '_userInfo', '_isTimingNow', '_unfinishedTime', '_unfinishedTimeIndex']),
      //确认是否上传数据
      confirmUpload() {
